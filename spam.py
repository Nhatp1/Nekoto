...
if __name__ == "__main__":
    ...
    delay = int(input("Nhập delay mỗi lần spam: "))
    dem_func = 0
    for i in range(1, count + 1):
        ...
        if (len (threads_quan_li) >=luong_chay ):
            while True:
                if (len (threads_quan_li) <luong_chay ):break
                for luong in threads_quan_li:
                    a_live = luong.is_alive()
                    if ((a_live)):
                        time.sleep (1)
                    else:
                        threads_quan_li.remove (luong)
        else:
            thread= threading.Thread(target=run, args=(phone,dem_func))
            thread.start()
            threads_quan_li.append (thread)
            time.sleep (delay)
        print(f"Spam thành công, {i}")
